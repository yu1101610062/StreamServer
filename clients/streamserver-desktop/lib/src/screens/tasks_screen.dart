          ...
          child: FilterBar(
            ...
            children: [
              ...
              SmallSelect(
                label: '类型',
                value: typeFilter,
                options: const ['', 'stream_ingest', 'stream_bridge', 'file_transcode'],
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: DataTable(
                      ...
                      rows: rows.map((row) {
                        return DataRow(
                          cells: [
                            DataCell(Text(textValue(row['name'])), onTap: () => controller.openTask('${row['id']}')),
                            DataCell(Text(textValue(row['type']))),
                            DataCell(Text(textValue(row['status']))),
                            DataCell(Text(shortId(row['assigned_node_id'] ?? row['node_id']))),
                            DataCell(Text(textValue(row['created_by']))),
                            DataCell(Text(textValue(row['updated_at']))),
                            DataCell(_TaskActions(row: row, onDone: _refresh)),
                          ],
                        );
                      }).toList(),
                    ),
...
class _TaskActions extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Wrap(
      ...
      runSpacing: 4,
      children: [
        TextButton(onPressed: () => controller.openTask(id), child: const Text('详情')),
        TextButton(onPressed: () => _mutate(context, controller, 'POST', '/api/v1/tasks/$id/start'), child: const Text('启动')),
        TextButton(
        ...
        TextButton(onPressed: () => _mutate(context, controller, 'POST', '/api/v1/tasks/$id/retry'), child: const Text('重试')),
        TextButton(onPressed: () => _mutate(context, controller, 'POST', '/api/v1/tasks/$id/clone'), child: const Text('克隆')),
        TextButton(
          onPressed: () => _confirmMutate(
            context,
            controller,
