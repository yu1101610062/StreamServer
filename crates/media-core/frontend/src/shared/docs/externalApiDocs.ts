...
const baseExternalApiDocs: ExternalApiDoc[] = [
  ...
  },
  {
    ...
    path: "{common.callback_url}",
    title: "接收任务完成回调",
    summary: "当任务进入终态，或后续有录像/转码产物补齐时，平台会向业务系统发送 `task.completed`。",
    description: "这同样是业务系统需要自行承接的接口。适合把任务完成、录像落盘和转码产物入库同步到你的业务系统。",
    successStatus: "2xx Accepted",
    implementationOwner: "business_system",
    direction: "平台 -> 业务系统",
    params: [
    ...
    notes: [
      ...
      "终态首次回调的 `reason` 通常是 `terminal_state`；如果录像或文件产物晚到，还可能追加一条 `reason=artifact_update` 的刷新回调。",
