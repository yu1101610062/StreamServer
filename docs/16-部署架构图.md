...
本文件偏“落地部署视角”。系统边界与文字说明可参考 [02-系统上下文与部署拓扑](./02-系统上下文与部署拓扑.md)，环境基线可参考 [11-环境准备与依赖基线](./11-环境准备与依赖基线.md)。
...
- 普通 HTTP/RTSP/RTMP 联调，允许使用 Docker bridge。
- 需要真实组播验证时，优先把 `media-agent` 与 `ZLMediaKit` 切到 `host` 或 `macvlan`。
- Compose 环境里的容器互访优先走服务名，如 `http://zlmediakit:80`，不建议绕宿主机映射端口。

## 7. 组播网络拓扑图

### 7.1 推荐的 host/macvlan 拓扑
